...
public class RightBoundaryHitboxComponent extends LineHitboxComponentAbs{

    private static final int DESTROY = 999;

    public RightBoundaryHitboxComponent(GameObject object, boolean enabled, Type objectType, Point2D p1, Point2D p2) {
    ...
    public void isColliding(GameObject object) {
        if (object.getComponent(HitboxComponent.class).get().getType() == Type.PROJECTILE) {
            ...
            object.setVelocity(new Vector2D(0, object.getVelocity().getyComp()));
        }
    }
}
