...
public class ShopModelImpl implements ShopModel{
    ...
    public Optional<Integer> check(int ID) {
        ...
        while(PWUiterator.hasNext()) {
            ...
            if(currPWU.getID()==ID) {
                    return (this.user.getMoney()-currPWU.getCost()>0) ? Optional.of(-currPWU.getCost()) : Optional.empty();
            }
                //se returna i soldi al negativo io posso toglierli con updateMoney
                //non fai il controllo se si può comprare perchè il bottone si disattiva automaticamente
        }
